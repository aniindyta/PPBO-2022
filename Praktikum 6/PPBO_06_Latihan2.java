...
public class PPBO_06_Latihan2 {
    public static void main(String[] args) {
        ...
        HashMap<String, Mahasiswa> mahasiswaHashMap = new HashMap<String, Mahasiswa>();
            for (Mahasiswa mahasiswa : mahasiswaArr) {
                mahasiswaHashMap.put(mahasiswa.NIM, mahasiswa);
            }

        for (Map.Entry<String, Mahasiswa> Mahasiswa : mahasiswaHashMap.entrySet()) {
            System.out.printf("key: %s => data mahasiswa: %s, %s, %d\n", 
            Mahasiswa.getKey(), Mahasiswa.getValue().nama, Mahasiswa.getValue().NIM, Mahasiswa.getValue().angkatan);
        }
